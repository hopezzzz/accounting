...
function getTrialBalanceBanksData($companyRef = NULL )
{
    ...
    $ci->db->select("transactions.transactionType,banks.bankRef,banks.name as bankName,sum(acct_transactionItems.amount + acct_transactionItems.vatAmount - acct_transactionItems.discountAmountPerItem - acct_transactionItems.commisionAmountPerItem) as totalAmount");
    ...
    $ci->db->select("transactions.transactionType,banks.bankRef,banks.name as bankName,sum(acct_payments.amount) as totalAmount");
    $ci->db->from('transactions');
    $ci->db->where('transactions.companyRef',$companyRef);
    ...
    if( !empty($salesBank) && !empty($salesBank1) )
    {
        foreach ($salesBank as $keyyyyy => $valueeeee)
        {
            $matchedkey = array_search($valueeeee->bankRef, array_column($salesBank, 'bankRef'));
            if( ( $matchedkey === 0 || $matchedkey  > 0 ) && $matchedkey != $keyyyyy)
            {
                $salesBank[$matchedkey]->totalAmount =  $salesBank[$matchedkey]->totalAmount + $valueeeee->totalAmount;
                unset($salesBank[$keyyyyy]);
            }
        }
    }
    $ci->db->select("transactions.transactionType,banks.bankRef,banks.name as bankName,sum(acct_transactionItems.amount + acct_transactionItems.vatAmount - acct_transactionItems.discountAmountPerItem - acct_transactionItems.commisionAmountPerItem) as totalAmount");
    ...
    if( !empty($purchaseExpenseBank) && !empty($purchaseExpenseBank1) )
    {
        foreach ($purchaseExpenseBank as $keyyyyy => $valueeeee)
        {
            $matchedkey = array_search($valueeeee->bankRef, array_column($purchaseExpenseBank, 'bankRef'));
            if( ( $matchedkey === 0 || $matchedkey  > 0 ) && $matchedkey != $keyyyyy)
            {
                $purchaseExpenseBank[$matchedkey]->totalAmount =  $purchaseExpenseBank[$matchedkey]->totalAmount + $valueeeee->totalAmount;
                unset($purchaseExpenseBank[$keyyyyy]);
            }
        }
    }


    $countSalesBank           = count($salesBank);
    $countPurchaseExpenseBank = count($purchaseExpenseBank);
    if( ( $countSalesBank == 1 && $countPurchaseExpenseBank == 1 ) && ( $salesBank[0]->bankRef != $purchaseExpenseBank[0]->bankRef))
    {
        $salesBank[0]->type             =  'db';
        $purchaseExpenseBank[0]->type   =  'cr';
        $loopingArray = array_merge($salesBank,$purchaseExpenseBank);
        return $loopingArray;
    }
    if( $countSalesBank > $countPurchaseExpenseBank )
    {
        $loopingArray  =  $salesBank;
        $seachingArray =  $purchaseExpenseBank;
        $type          =  'sales';
    }
    else
    {
        $loopingArray  =  $purchaseExpenseBank;
        $seachingArray =  $salesBank;
        $type          =  'p&e';
    }
    foreach ($loopingArray as $key => $value)
    {
        $matchedkey = array_search($value->bankRef, array_column($seachingArray, 'bankRef'));
        if( $matchedkey === 0 || $matchedkey  > 0)
        {
            if( $seachingArray[$matchedkey]->totalAmount > $value->totalAmount )
            {
                $loopingArray[$key]->totalAmount =  $seachingArray[$matchedkey]->totalAmount - $value->totalAmount;
                if( $type == 'sales' )
                    $loopingArray[$key]->type        =  'cr';
                else
                    $loopingArray[$key]->type        =  'db';
            }
            else if( $value->totalAmount > $seachingArray[$matchedkey]->totalAmount  )
            {
                $loopingArray[$key]->totalAmount =  $value->totalAmount - $seachingArray[$matchedkey]->totalAmount ;
                if( $type == 'sales' )
                    $loopingArray[$key]->type        =  'db';
                else
                    $loopingArray[$key]->type        =  'cr';
            ...
            {
                $loopingArray[$key]->totalAmount =  0;
                $loopingArray[$key]->type        =  'cr';
            }
        }
        else
        {
            $loopingArray[$key]->totalAmount =  $value->totalAmount;
            if( $type == 'sales' )
                $loopingArray[$key]->type        =  'db';
            else
                $loopingArray[$key]->type        =  'cr';
    ...
    if($query->num_rows() > 0)
    {
        ...
        foreach( $result as $key=>$value)
        {
            ...
            else
            {
                ...
                if($query1->num_rows() > 0)
                {
                    if( $value->categoryRef == 'MsVfObrT4PI1QmGV') /// purchase return in expense will be credited.
                        $result[$key]->type = 'cr';
                    $result[$key]->totalAmount = $query1->row()->totalAmount;
                    if($query1->row()->totalAmount > 0)
                        $result[$key]->hasTransactions = true;
